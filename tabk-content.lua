...
function TabContents:Initialize(UI)
    ...
    csb("Main", "farm", function()
        ...
        -- EGG HATCH SPEED TOGGLE (SIMPLE - NO SPAM)
        ctg("🥚 Egg Hatch Speed Boost (x2)", false, function(v)
            local success, message = PickaxeFeatures:SetEggHatchSpeed(v and 2 or 1)
            if success then
                showNotif(v and "🥚 Boosted" or "⚪ Reset", message, 2)
            end
        end)
    ...
    csb("Visual", "visual", function()
        ...
        local nametagYPos = 3 -- Default Y position
